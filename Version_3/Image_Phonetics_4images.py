...
# Create a dictionary to map i to k
i_to_k_mapping = {1: 2, 2: 1, 3: 4, 4: 3}

# Directory containing the images
image_dir = os.path.join(os.path.dirname(__file__), "./PDF_generation/Phonetics")
...
for i in range(0, len(image_list), images_per_a4):
    ...
    for j in range(images_per_a4):
        if i + j < len(image_list):
            ...
            k = i_to_k_mapping.get(j + 1, i + 1)  # Default to i + 1 if not found in mapping  
			# Calculate the position to paste the image at the center of the cell
            cell_x = ((k - 1) % num_columns) * cell_width
            cell_y = ((k - 1) // num_columns) * cell_height
